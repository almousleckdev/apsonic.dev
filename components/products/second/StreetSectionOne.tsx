...
export const StreetSectionOne: React.FC<StreetSectionOneProps> = ({
  product,
}) => {
  const [selectedColor, setSelectedColor] = useState<ColorVariant>("white");
  const activeVariant =
    COLOR_VARIANTS.find((v) => v.id === selectedColor) ?? COLOR_VARIANTS[0];
    ...
    <section className="py-12 sm:py-16 bg-white overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 z-20 flex items-center gap-6">
              <motion.span
                ...
                className="text-[clamp(4rem,12vw,8rem)] font-bold text-gray-300/35 tracking-tight leading-none select-none pointer-events-none whitespace-nowrap"
              >
                {product.model}
              </motion.span>
            ...
            <motion.div
              ...
              className="absolute inset-0 z-10 flex items-start justify-center pt-12 sm:pt-16"
            >
              <Image
        ...
        <motion.p
          ...
          className="mt-8 text-center text-xs sm:text-sm text-gray-600"
        >
          {SALES_REGIONS}
        </motion.p>
      </div>
