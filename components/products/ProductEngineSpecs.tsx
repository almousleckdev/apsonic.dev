...
export const ProductEngineSpecs: React.FC<ProductEngineSpecsProps> = ({
  ...
  return (
    <section
      className={cn(
        "relative w-full h-auto min-h-[500px] sm:min-h-[600px] flex items-center overflow-hidden bg-gray-50 py-20 sm:py-0 sm:h-screen",
        className
      )}
    >
      ...
      <div className="absolute inset-0 w-full h-full z-0">
        <Image
          ...
          fill
          priority
          className="object-cover object-center lg:object-right opacity-40 blur-sm"
          sizes="100vw"
        />
      ...
      <div className="container mx-auto px-6 sm:px-10 lg:px-16 relative z-20">
        <div className="max-w-xl flex flex-col items-start gap-8 sm:gap-12">
          ...
          >
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-medium text-gray-900 tracking-wide">
              {title}
            </h2>
          ...
          >
            {specs.map((spec, index) => (
              <MotionDiv
                key={index}
                className="bg-white/50 backdrop-blur-sm border border-gray-100 rounded-lg px-6 py-5 sm:py-8 flex items-center justify-between group hover:bg-white hover:shadow-lg hover:shadow-gray-200 transition-all duration-300"
              ...
              >
                <span className="text-xs sm:text-sm text-gray-500 font-light tracking-wider uppercase">
                  {spec.label}
                </span>
                <span className="text-xl sm:text-2xl md:text-3xl font-medium text-brand-green tracking-tight">
                  {spec.value}
                </span>
              </MotionDiv>
            ))}
