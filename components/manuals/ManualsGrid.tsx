...
export const ManualsGrid = () => {
    return (
        <div className="py-16 container mx-auto px-4">
            <h2 className="text-2xl font-bold text-white mb-8">Latest Documentation</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {manuals.map((manual, idx) => (
                    <motion.div
                        ...
                        className="bg-[#151515] border border-white/5 rounded-xl p-6 hover:bg-[#1a1a1a] hover:border-brand-green/30 transition-all group cursor-pointer"
                    >
                        <div className="flex items-start justify-between mb-4">
                            <div className="h-10 w-10 bg-red-900/20 text-red-500 rounded-lg flex items-center justify-center text-xl">
                                <MdPictureAsPdf />
                            </div>
                            <span className="text-xs text-gray-500 font-mono bg-[#222] px-2 py-1 rounded">{manual.model}</span>
                        </div>

                        <h3 className="text-white font-bold text-lg mb-1 group-hover:text-brand-green transition-colors">{manual.name}</h3>
